...
//     e.preventDefault();
//     toggleWishlist(product);
//     // Add a subtle notification for wishlist
//     if (isSaved) {
//       toast.info('Removed from wishlist', { autoClose: 1500 });
//     } else {
//       toast.success('❤️ Added to wishlist!', { autoClose: 1500 });
...
//       <button 
//         onClick={handleWishlistToggle}
//         className="absolute top-3 right-3 z-10 p-2 rounded-full bg-white/80 backdrop-blur-sm shadow-sm hover:bg-white transition-all hover:scale-110"
//       >
//         {isSaved ? '❤️' : '🤍'}
//       </button>

//       {/* Product Image */}
//       <Link to={`/products/${product.id}`} className="block relative h-64 overflow-hidden p-4 group">
//         <img 
//           src={product.image} 
//           alt={product.title} 
...
//         <Link to={`/products/${product.id}`}>
//           <h2 className="text-lg font-semibold text-gray-800 line-clamp-2 mb-2 hover:text-blue-600 transition-colors">
//             {product.title}
//           </h2>
//         </Link>
...
//           <span className="text-yellow-400 text-sm">★ {product.rating?.rate}</span>
//           <span className="text-gray-400 text-xs ml-2">({product.rating?.count})</span>
//         </div>
...
//           <button 
//             className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors duration-200 active:scale-95"
//             onClick={() => {
//               addToCart(product);
//               // <-- 2. The sleek new Toast notification!
